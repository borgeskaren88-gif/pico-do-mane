...
export function qtdNaUnidadeDoItem(qtd, unidadeReceita, item) {
  ...
  // O SACO SEPARADO JÁ DIZ QUANTO TEM DENTRO.
  //
  // Um item porcionado carrega "cada saco leva 300 g" em `separar`. Isso é a
  // mesma informação que o "conteúdo por unidade", só escrita noutro campo —
  // e sem ligar os dois, uma ficha pedindo "100 g" de "Carne 300g" caía no
  // chute do fim da função e baixava CEM SACOS.
  //
  // O conteúdo explícito manda, se ela tiver preenchido os dois: foi ela que
  // digitou, e o campo existe justamente pra mandar.
  const sep = item && item.separar;
  const temConteudoProprio = num(item?.conteudo) > 0 && !!item?.conteudoUnid;
  const conteudo = temConteudoProprio ? num(item.conteudo)
    : (sep && num(sep.gramas) > 0 ? num(sep.gramas) : 0);
  const uConteudo = temConteudoProprio ? item.conteudoUnid
    : (sep && num(sep.gramas) > 0 ? (sep.unidade || 'g') : undefined);
...
export function conversaoDaReceita(unidadeReceita, item) {
  ...
  // O SACO SEPARADO tambem diz quanto tem dentro, so que em `separar`.
  //
  // qtdNaUnidadeDoItem ja converte por ali — "100 g" de um saco de 300 g da um
  // terco. Este diagnostico nao sabia, e acusava o item de estar sem conversao:
  // um alarme vermelho mandando ela "arrumar" o que ja estava certo. E o
  // conserto sugerido (trocar a unidade do item pra g) quebraria os dois
  // freezers, os minimos e a contagem inteira.
  const sep = item && item.separar;
  if (sep && num(sep.gramas) > 0 && fatorMesmaFamilia(u, sep.unidade || 'g') != null) {
    return { ok: true, via: 'porcao' };
  }
  return {
    ok: false,
    via: 'chute',
