...
export function recalcularCustosPelasCompras(estoque, compras) {
  ...
  const novo = estoque.map((it) => {
    ...
    // Média ponderada das compras dos últimos 90 dias, e não o preço da última.
    // Pegar só a última faria esta correção desfazer o custo médio: bastava uma
    // compra cara de emergência pra ela reprecificar tudo de novo.
    const desde = addDays(todayISO(), -90);
    ...
    const novoCusto = pesoTotal > 0 ? Math.round((valorTotal / pesoTotal) * 10000) / 10000 : precoDe(doItem[0]);
