...
export function comprasPendentesDeEstoque(compras, estoque, dias = 45, hoje = todayISO()) {
  ...
  for (const c of (Array.isArray(compras) ? compras : [])) {
    ...
    if (!(numQtd(c.quantidade) > 0)) continue;
...
export function fundirItens(estoque, fichas, compras, principalId, absorvidosIds) {
  ...
  // unidade viram uma só somada; unidades diferentes ficam como estão, porque
  // o cálculo já sabe somar linha por linha.
  const novasFichas = (Array.isArray(fichas) ? fichas : []).map((f) => {
  ...
  const novasCompras = (Array.isArray(compras) ? compras : []).map((c) =>
    (c && mortos.has(c.estoqueId) ? { ...c, estoqueId: principalId } : c));

  return { estoque: novoEstoque, fichas: novasFichas, compras: novasCompras, fundidos: absorvidos.length, naoDeu };
}
...
export function aplicarEntradasEstoque(estoque, comprasNovas) {
  ...
  for (const c of novas) {
    if (!c || numQtd(c.quantidade) <= 0) continue;
    const r = resolverCompraNoEstoque(c, itens);
    if (!r) continue;
