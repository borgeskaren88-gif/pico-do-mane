...
// Lê os próximos eventos da agenda principal do Google (para exibir no app).
...
export async function listarEventos() {
  ...
  const params = new URLSearchParams({
    timeMin: agora.toISOString(), timeMax: timeMax.toISOString(),
    singleEvents: 'true', orderBy: 'startTime', maxResults: '20',
  });
  const res = await fetch('https://www.googleapis.com/calendar/v3/calendars/primary/events?' + params.toString(), {
    headers: { Authorization: 'Bearer ' + accessToken },
  });
  if (!res.ok) throw new Error('Erro ao listar eventos: ' + (await res.text()));
  const j = await res.json();
  return (j.items || []).map((e) => ({
    id: e.id,
    titulo: e.summary || '(sem título)',
    inicio: (e.start && (e.start.dateTime || e.start.date)) || '',
    diaTodo: !(e.start && e.start.dateTime),
  })).filter((e) => e.inicio);
}
