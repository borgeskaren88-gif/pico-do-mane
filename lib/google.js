...
export async function listarEventos() {
  ...
  try {
    // showCompleted+showHidden traz também as concluídas (que o Google "esconde"),
    // pra elas aparecerem riscadas na agenda em vez de sumir.
    const resT = await fetch('https://www.googleapis.com/tasks/v1/lists/@default/tasks?showCompleted=true&showHidden=true&maxResults=100', { headers: { Authorization: 'Bearer ' + accessToken } });
    if (resT.ok) {
      ...
      for (const t of (jt.items || [])) {
        ...
        itens.push({ id: 'task-' + t.id, titulo: t.title || '(tarefa)', inicio: d, diaTodo: true, tarefa: true, concluida: t.status === 'completed' });
      }
    }
...
export async function criarEvento({ titulo, data, hora, diaTodo }) {
  ...
  let body;
  if (diaTodo) {
    // Dia inteiro: usa o lembrete padrão do Calendário (evento sem horário exato).
    body = { summary: nome, start: { date: data }, end: { date: addDays(data, 1) }, reminders: { useDefault: true } };
  } else {
    const h = /^\d{2}:\d{2}$/.test(hora || '') ? hora : '12:00';
    // Com horário: avisa 10 min antes E na hora exata (o app Calendário dispara).
    body = { summary: nome, start: { dateTime: `${data}T${h}:00-03:00`, timeZone: TZ }, end: { dateTime: maisUmaHora(data, h), timeZone: TZ }, reminders: { useDefault: false, overrides: [{ method: 'popup', minutes: 10 }, { method: 'popup', minutes: 0 }] } };
  }
...
// Mexe num evento que o PicoOS já criou (a reserva que mudou de dia ou de
// hora). Devolve o corpo do evento do jeito que o Google monta — os dois
// caminhos (dia inteiro e com horário) são os mesmos do criarEvento.
function corpoEvento({ titulo, data, hora, diaTodo }) {
  const nome = String(titulo || '').trim().slice(0, 200) || '(sem título)';
  if (diaTodo) return { summary: nome, start: { date: data }, end: { date: addDays(data, 1) }, reminders: { useDefault: true } };
  const h = /^\d{2}:\d{2}$/.test(hora || '') ? hora : '12:00';
  return {
    summary: nome,
    start: { dateTime: `${data}T${h}:00-03:00`, timeZone: TZ },
    end: { dateTime: maisUmaHora(data, h), timeZone: TZ },
    reminders: { useDefault: false, overrides: [{ method: 'popup', minutes: 10 }, { method: 'popup', minutes: 0 }] },
  };
}

export async function atualizarEvento(id, { titulo, data, hora, diaTodo }) {
  const accessToken = await accessTokenValido();
  if (!accessToken) throw new Error('Google não conectado.');
  if (!id) throw new Error('Evento não informado.');
  if (!data || !/^\d{4}-\d{2}-\d{2}$/.test(data)) throw new Error('Data inválida.');
  const res = await calAPI('/' + encodeURIComponent(id), 'PATCH', corpoEvento({ titulo, data, hora, diaTodo }), accessToken);
  if (!res.ok) throw new Error('Erro ao atualizar o evento no Google: ' + (await res.text()));
  return await res.json();
}
...
export async function apagarEvento(id) {
  ...
  if (!accessToken) throw new Error('Google não conectado.');
  if (!id) throw new Error('Evento não informado.');
  const res = await calAPI('/' + encodeURIComponent(id), 'DELETE', null, accessToken);
  // 410 = o Google já tinha apagado. Pra gente dá no mesmo: o evento não está lá.
  if (!res.ok && res.status !== 404 && res.status !== 410) throw new Error('Erro ao apagar o evento no Google.');
  return true;
}
...
export async function concluirTarefa(taskId, concluida = true) {
  const accessToken = await accessTokenValido();
  if (!accessToken) throw new Error('Google não conectado.');
  const id = String(taskId || '').trim();
  if (!id) throw new Error('Tarefa não informada.');
  // Concluir = status 'completed'; desmarcar = volta pra 'needsAction' e limpa a
  // data de conclusão (senão o Google mantém ela como feita).
  const body = concluida ? { status: 'completed' } : { status: 'needsAction', completed: null };
  const res = await tasksAPI('/' + encodeURIComponent(id), 'PATCH', body, accessToken);
  if (!res.ok) throw new Error('Erro ao atualizar a tarefa no Google: ' + (await res.text()));
  return true;
}
