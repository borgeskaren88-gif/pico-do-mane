...
let _perguntando = null;
export async function vozExclusivaDisponivel() {
  if (_disponivel !== null) return _disponivel;
  if (_perguntando) return _perguntando;
  _perguntando = (async () => {
    try {
      const r = await fetch('/api/darci/voz', { cache: 'no-store', signal: AbortSignal.timeout(6000) });
      const j = await r.json();
...
// Pergunta ANTES de precisar. Isso é o que permite decidir na hora da fala sem
// esperar a rede — ver o comentário em falarTexto.
export function prepararVoz() {
  if (typeof window === 'undefined') return;
  destravarNoPrimeiroToque();
  vozExclusivaDisponivel().catch(() => { /* fica a voz do aparelho */ });
}

// Testa a voz DE VERDADE: manda o serviço falar uma frase curta e conta o que
// aconteceu. O `vozExclusivaDisponivel` acima só sabe dizer se existe chave —
// e chave existindo mas recusada dá voz do aparelho em silêncio, sem ninguém
// entender por quê. Este aqui diz o porquê.
export async function testarVoz() {
  try {
    const r = await fetch('/api/darci/voz?teste=1', { cache: 'no-store', signal: AbortSignal.timeout(25000) });
    let j = null;
    try { j = await r.json(); } catch { /* veio outra coisa */ }
    if (!j) return { estado: 'erro', erro: `O servidor respondeu ${r.status} sem explicação.` };
    if (j.semChave) return { estado: 'sem-chave', onde: j.onde || null };
    return {
      estado: j.ok ? 'ok' : 'erro', motor: j.motor, onde: j.onde || null,
      ms: j.ms, erro: j.erro || '', cru: j.cru || '',
    };
  } catch (e) {
    const t = /Timeout|Abort/i.test(e?.name || '') ? 'O teste da voz passou de 25 segundos sem resposta.' : 'Não consegui chegar no servidor.';
...
// Destrava o som. TEM que ser chamado DENTRO do toque da dona — é a única
// janela em que o navegador deixa. Depois de destravado uma vez, o mesmo
// tocador pode falar a qualquer momento na sessão, inclusive quando a resposta
// da IA chega três segundos depois.
export function destravarAudio() {
  if (_destravado || typeof window === 'undefined') return;
  _destravado = true;
  const a = tocador();
  if (a) {
    try { a.src = SILENCIO; const p = a.play(); if (p && p.catch) p.catch(() => { /* sem problema */ }); } catch { /* ignora */ }
  }
  // A voz do aparelho tem a mesma trava. Uma fala vazia acorda o motor sem
...
// Destrava no PRIMEIRO toque que acontecer no app, seja onde for.
//
// Antes o destrave só era chamado na hora de falar — e a fala vem depois da
// resposta da IA, segundos depois do toque. A janela do navegador já tinha
// fechado, e o Darci respondia escrito e ficava calado. Agora qualquer toque
// serve, e ele já chega destravado na hora de responder.
let _vigiaToque = false;
function destravarNoPrimeiroToque() {
  if (_vigiaToque || typeof window === 'undefined') return;
  _vigiaToque = true;
  const solta = () => {
    destravarAudio();
    window.removeEventListener('pointerdown', solta);
    window.removeEventListener('keydown', solta);
  };
  window.addEventListener('pointerdown', solta, { once: true });
...
async function falarNaNuvem(texto, aoIniciar, aoTerminar) {
  ...
  let url = '';
  try {
    // Com prazo: serviço de voz lento ou pendurado deixava o Darci mudo pra
    // sempre, porque a volta pra voz do aparelho só acontece quando esta
    // promessa termina.
    const r = await fetch('/api/darci/voz', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ texto }), signal: AbortSignal.timeout(12000),
    });
...
//
// A ORDEM AQUI É O QUE FAZ ELE FALAR OU FICAR MUDO.
//
// O navegador só deixa tocar som logo depois de um toque da pessoa. Esse
// "logo depois" é curto: se a gente for na rede perguntar se existe voz na
// nuvem e só então mandar falar, o toque já expirou, e aí NADA sai — nem a voz
// da nuvem nem a do aparelho. Era isso que estava acontecendo: o Darci
// respondia escrito e ficava calado.
//
// Então: quando ainda não se sabe se existe voz na nuvem, ele fala JÁ com a voz
// do aparelho e pergunta em paralelo, pra próxima vez sair com a voz boa.
export function falarTexto(texto, { vozId, tom, nome, sotaque, aoIniciar, aoTerminar } = {}) {
  ...
  const noAparelho = () => falarNoAparelho(pronto, { vozId, tom, aoIniciar, aoTerminar });
  if (typeof window === 'undefined' || lerMotorVoz() !== 'exclusiva') { noAparelho(); return; }

  pararFala();
  if (_disponivel === null) { prepararVoz(); noAparelho(); return; }
  if (_disponivel === false) { noAparelho(); return; }
  falarNaNuvem(pronto, aoIniciar, aoTerminar)
    .then((deu) => { if (!deu) noAparelho(); })
    .catch(() => noAparelho());
}

// O navegador demora pra montar a lista de vozes — e no começo ela vem vazia.
// Quem falasse nessa janela saía com a voz padrão do sistema (uma), e quem
// falasse depois saía com a voz escolhida (outra). Era essa a troca de voz
// entre o balão e a tela do Darci. Agora ninguém fala antes da lista existir.
function comVozes(cb, tentativa = 0) {
  const lista = listarVozes();
  if (lista.length || tentativa >= 12) { cb(lista); return; }
  setTimeout(() => comVozes(cb, tentativa + 1), 120);
}

// A voz de fábrica do aparelho — o jeito que sempre funcionou.
function falarNoAparelho(pronto, opcoes = {}) {
  if (!temVoz()) { if (opcoes.aoTerminar) opcoes.aoTerminar(); return; }
  try { window.speechSynthesis.cancel(); } catch { /* ignora */ }
  comVozes((lista) => dizer(pronto, lista, opcoes));
}

function dizer(pronto, lista, { vozId, tom, aoIniciar, aoTerminar } = {}) {
  try {
    ...
    try {
      // A voz pedida só vale se ela EXISTE neste aparelho (a da Microsoft, por
      // exemplo, só existe no Windows). Se não existir, todo mundo — balão e
      // tela — cai na mesma escolha, e a voz não muda mais de um lugar pro
      // outro.
      const pedida = vozId && lista.some((x) => x.voiceURI === vozId) ? vozId : vozPadrao(lista);
      const v = lista.find((x) => x.voiceURI === pedida);
    ...
    // Dois defeitos conhecidos do Chrome, os dois deixam a fala pela metade ou
    // nenhuma:
    //   - depois de um cancel(), ele às vezes fica "pausado" e engole a fala
    //     seguinte. resume() destrava.
    //   - fala longa é cortada por volta de 15 segundos, a não ser que alguém
    //     cutuque ele de tempos em tempos.
    let cutuca = null;
    const pararCutuca = () => { if (cutuca) { clearInterval(cutuca); cutuca = null; } };
    let comecou = false;

    u.onstart = () => { comecou = true; if (aoIniciar) aoIniciar(); };
    u.onend = () => { pararCutuca(); if (aoTerminar) aoTerminar(); };
    u.onerror = () => { pararCutuca(); if (aoTerminar) aoTerminar(); };

    window.speechSynthesis.speak(u);
    cutuca = setInterval(() => {
      try {
        const s = window.speechSynthesis;
        if (!s.speaking && !s.pending) { pararCutuca(); return; }
        s.resume();
      } catch { pararCutuca(); }
    }, 5000);
    // Se em 1,5s não saiu som nenhum, quase sempre é o estado "pausado".
    setTimeout(() => { if (!comecou) { try { window.speechSynthesis.resume(); } catch { /* ignora */ } } }, 1500);
