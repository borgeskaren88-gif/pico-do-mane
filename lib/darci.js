...
export function analisarBar({ receitas = [], despesas = [], compras = [], vendas = [], estoque = [], tarefas = [], clientes = [], cardapio = [], fichas = [], desdeMs = 0 } = {}) {
  ...
  // O ciclo do fiado: consumo de um mês se paga do dia 01 ao 10 do mês seguinte.
  // Então o que é de mês passado já venceu; o deste mês só cai mês que vem.
  const aReceberAgora = Math.round(vendas.filter((v) => String((v.data || v.fechadaEm) || '').slice(0, 7) < ym).reduce((s, v) => s + abertoDaVenda(v), 0) * 100) / 100;
...
function responderCru(texto, n) {
  ...
  if (tem(t, 'receber', 'fiado', 'devendo', 'deve', 'cobrar', 'devedor')) {
    ...
    const ciclo = n.aReceberAgora > 0.005 && n.aReceberProximo > 0.005
      ? ` Desse total, ${brl(n.aReceberAgora)} ${n.diaDoMes <= 10 ? 'vence até o dia 10' : 'já venceu no dia 10'} e ${brl(n.aReceberProximo)} é consumo deste mês, que só cai mês que vem.`
      : n.aReceberAgora > 0.005
        ? ` Tudo isso ${n.diaDoMes <= 10 ? 'vence até o dia 10' : 'já passou do dia 10 — está atrasado'}.`
        : ' É tudo consumo deste mês, que só cai do dia 01 ao 10 do mês que vem.';
