...
export function interpretarComando(texto, { estoque = [], despesas = [] } = {}) {
  ...
  // 1) Perda/baixa no estoque: precisa achar o item pelo nome.
  const mPerda = /(?:d[aá]\s+)?baixa|perd[ai]|quebr|estrag|joguei fora|venceu|estragou/.test(t);
  if (mPerda) {
    const q = /(\d+[.,]?\d*)/.exec(t);
    const itens = arr(estoque)
    ...
    if (itens.length) {
      ...
      const qtd = q ? numeroBR(q[1]) : 1;
      const it = itens[0].it;
      return {
        tipo: 'perda',
        resumo: `Baixar ${qtdBR(qtd)} ${it.unidade || 'un'} de ${limparNome(it.nome)} do estoque, como ${motivo.toLowerCase()}.`,
        dados: { itemId: it.id, nome: limparNome(it.nome), qtd, motivo, unidade: it.unidade || 'un' },
      };
    }
  }

  // 2) Despesa: tem valor em dinheiro e uma descrição.
  const mDesp = /(?:lan[cç]a|anota|registra|paguei|pagamos|gastei|gastamos|comprei|compramos|foi|deu)\s+(?:r\$\s*)?(\d+[.,]?\d*)\s*(?:reais?|conto|pila)?\s*(?:de|em|com|no|na|pro|pra|do|da|para)?\s*(.*)$/.exec(t);
  if (mDesp && numeroBR(mDesp[1]) > 0) {
    const valor = numeroBR(mDesp[1]);
    // Pega a descrição do texto ORIGINAL (com acento e maiúscula certos).
    const posicao = cru.toLowerCase().lastIndexOf(String(mDesp[2] || '').slice(0, 6).toLowerCase());
    const descricao = maiuscula(limparNome(mDesp[2] && posicao >= 0 ? cru.slice(posicao) : mDesp[2] || ''));
    if (descricao) {
      // Reaproveita a categoria que ela já usou pra uma despesa parecida.
      const parecida = arr(despesas).find((d) => d && d.descricao && norm(d.descricao).includes(norm(descricao).slice(0, 6)));
      const categoria = (parecida && parecida.categoria) || 'A classificar';
      return {
        tipo: 'despesa',
        resumo: `Lançar despesa de ${brl(valor)} — ${descricao} (${categoria}), hoje.`,
        dados: { valor, descricao, categoria },
      };
    }
  }

  // 3) Tarefa: o resto do que começa com "anota/lembra/preciso".
  const mTar = /^(?:anota|anotar|lembra|lembre|lembrete|preciso|tenho que|tarefa|bota na lista|p[oõ]e na lista)\s+(?:que\s+|de\s+|do\s+|da\s+|pra\s+|para\s+)?(.+)$/i.exec(cru);
  if (mTar && mTar[1].trim().length > 2) {
    const texto2 = maiuscula(limparNome(mTar[1]));
    return { tipo: 'tarefa', resumo: `Anotar no TO DO: "${texto2}".`, dados: { texto: texto2 } };
  }
