...
import { num, brl, addDays, ymOf, limparNome, fiadoDaVenda, abertoDaVenda, diaOperacional, weekday, classificarFiado } from './util';
import { custoDaFicha, custoDosSabores, itensVencendo, textoValidade } from './estoque';
...
// Dia da semana como número (0 = domingo), e o nome curto do jeito que a gente
// fala: "sexta", não "Sexta-Feira".
const diaSemanaNum = (iso) => { try { return new Date(iso + 'T12:00:00').getDay(); } catch { return 0; } };
const nomeDia = (iso) => weekday(iso).replace('-Feira', '').toLowerCase();
// "hoje", "amanhã", "sexta" ou "12/10" — como a gente falaria a data em voz alta.
const quandoFalado = (d, hoje) => {
  if (d === hoje) return 'hoje';
  if (d === addDays(hoje, 1)) return 'amanhã';
  if (d > hoje && d <= addDays(hoje, 6)) return nomeDia(d);
  return `${d.slice(8, 10)}/${d.slice(5, 7)}`;
};
// Quanto por cento uma coisa está acima (+) ou abaixo (−) da outra.
const pctDe = (atual, base) => (base > 0 ? Math.round(((atual - base) / base) * 100) : 0);
...
// Começo da resposta de quando ele não entendeu. A tela olha por isso pra
// mostrar as sugestões prontas — assim a dona toca em vez de brigar com o
// ditado do teclado.
export const NAO_ENTENDI = 'Essa eu ainda não sei responder.';

export const ATALHOS = [
  'O que mudou?',
  'Briefing do dia',
  'Previsão de hoje',
  'Ontem foi bom ou ruim?',
  'O que vai acabar?',
  'O que está vencendo?',
  'Qual meu melhor dia?',
  'Como foi ontem?',
...
export function analisarBar({ receitas = [], despesas = [], compras = [], vendas = [], estoque = [], tarefas = [], clientes = [], cardapio = [], fichas = [], reservas = [], desdeMs = 0 } = {}) {
  ...
  cardapio = arr(cardapio); fichas = arr(fichas); reservas = arr(reservas);
  ...
  // O fiado fecha no fim do mês. Quem tem conta do mês passado em aberto paga
  // tudo agora (dia 01 ao 10); quem só consumiu depois do dia 1º fica pro próximo.
  const idadeFiado = classificarFiado(vendas, ym);
  const aReceberAgora = Math.round(vendas.filter((v) => idadeFiado.balde.get(v.id) !== 'proximo').reduce((s, v) => s + abertoDaVenda(v), 0) * 100) / 100;
  const aReceberProximo = Math.round((aReceber - aReceberAgora) * 100) / 100;
  const diaDoMes = Number(hoje.slice(8, 10)) || 1;
  ...
  // Salário/vale/folha é conta que não pode atrasar um dia: entra separada.
  const ehSalario = (c) => /sal[aá]rio|folha|vale |vale-|funcion|diarist|pagamento pessoal/i.test(`${c.produto || ''} ${c.fornecedor || ''} ${c.categoria || ''}`);
  const salarios = abertas.filter((c) => ehSalario(c) && c.vencimento <= addDays(hoje, 3));
  ...
  // Os últimos dias da semana iguais a um dia qualquer (as últimas oito sextas,
  // se a base é uma sexta). Serve pra prever hoje E pra dizer se ontem foi bom.
  const diasIguaisA = (base, quantos = 8) => {
    const out = [];
    for (let i = 7; out.length < quantos && i <= quantos * 7 + 28; i += 7) {
      ...
      const pes = vs.reduce((s2, v) => s2 + (num(v.pessoas) || 0), 0);
      out.push({ data: d, caixa: cx, fiado: fi, total: cx + fi, pedidos: vs.length, pessoas: pes, itens: vs });
    }
    return out;
  };
  const diasIguais = diasIguaisA(hoje, 8);
  ...
  const previsao = {
    ...
    // Gente esperada: só vale quando alguém marcou de verdade quantas pessoas
    // eram (se deu uma por mesa, é o padrão do sistema, não contagem).
    pessoas: (() => {
      const bons = diasIguais.filter((d) => d.pessoas > 0 && !(d.pessoas === d.pedidos && d.pedidos > 1));
      return bons.length ? Math.round(media(bons.map((d) => d.pessoas))) : 0;
    })(),
  ...
  // ===========================================================================
  // 1. COMPARAR COM O PASSADO
  // Número sozinho não diz nada: R$ 1.400 num sábado é ruim, numa terça é ótimo.
  // Aqui cada número ganha uma referência — o mesmo dia da semana, a semana
  // anterior, o mesmo pedaço do mês passado.
  // ===========================================================================
  const fiadoDe = (d) => vendas.filter((v) => (v.data || '') === d).reduce((s, v) => s + fiadoDaVenda(v), 0);
  const totalDia = (d) => caixaDe(d) + fiadoDe(d);
  const cent = (v) => Math.round(v * 100) / 100;

  // Ontem foi bom ou ruim? Compara com os outros dias iguais a ontem.
  const iguaisOntem = diasIguaisA(ontem, 6);
  const mediaOntem = cent(media(iguaisOntem.map((d) => d.total)));
  const totalOntemN = cent(caixaOntem + fiadoOntem);
  const cmpOntem = (iguaisOntem.length >= 2 && mediaOntem > 0 && totalOntemN > 0)
    ? { dia: nomeDia(ontem), total: totalOntemN, media: mediaOntem, dias: iguaisOntem.length, pct: pctDe(totalOntemN, mediaOntem) }
    : null;

  // Últimos sete dias fechados contra os sete anteriores.
  const somaDias = (ini, fim) => { let s = 0; for (let d = ini; d <= fim; d = addDays(d, 1)) s += totalDia(d); return cent(s); };
  const sem1 = somaDias(addDays(ontem, -6), ontem);
  const sem2 = somaDias(addDays(ontem, -13), addDays(ontem, -7));
  const cmpSemana = sem2 > 0 ? { atual: sem1, antes: sem2, pct: pctDe(sem1, sem2) } : null;

  // Este mês, do dia 1 até hoje, contra o MESMO pedaço do mês passado — senão
  // comparar mês começado com mês inteiro engana, e engana sempre pra pior.
  const primeiroDoMes = `${ym}-01`;
  const fimDoMesPassado = addDays(primeiroDoMes, -1);
  const ymAnt = ymOf(fimDoMesPassado);
  const mesmoDiaAnt = `${ymAnt}-${hoje.slice(8, 10)}`;
  const fimAnt = mesmoDiaAnt > fimDoMesPassado ? fimDoMesPassado : mesmoDiaAnt;
  const somaRec = (ini, fim) => receitas.filter((r) => r && r.data >= ini && r.data <= fim && (r.categoria || '') !== ATRASADO).reduce((s, r) => s + num(r.valor), 0);
  const somaDesp = (ini, fim) => despesas.filter((d) => d && d.data >= ini && d.data <= fim).reduce((s, d) => s + num(d.valor), 0);
  const recMesAte = cent(somaRec(primeiroDoMes, hoje));
  const despMesAte = cent(somaDesp(primeiroDoMes, hoje));
  const recAnt = cent(somaRec(`${ymAnt}-01`, fimAnt));
  const despAnt = cent(somaDesp(`${ymAnt}-01`, fimAnt));
  const cmpMes = (recAnt > 0 || despAnt > 0)
    ? { recAnt, despAnt, resultadoAnt: cent(recAnt - despAnt), rec: recMesAte, desp: despMesAte, pctRec: pctDe(recMesAte, recAnt), pctDesp: pctDe(despMesAte, despAnt), ateDia: Number(hoje.slice(8, 10)) || 1 }
    : null;

  // O melhor e o pior dia da semana, na média das últimas oito semanas. É o tipo
  // de coisa que dono de bar leva anos pra enxergar sozinho.
  const NOMES_DIA = ['domingo', 'segunda', 'terça', 'quarta', 'quinta', 'sexta', 'sábado'];
  const porDiaSemana = NOMES_DIA.map((nome, idx) => {
    const ts = [];
    for (let i = 1; i <= 56; i++) {
      const d = addDays(hoje, -i);
      if (diaSemanaNum(d) !== idx) continue;
      const t2 = totalDia(d);
      // Dia sem nada lançado é dia fechado, não dia ruim — não entra na média.
      if (t2 > 0 || vendas.some((v) => (v.data || '') === d)) ts.push(t2);
    }
    return { dia: nome, idx, dias: ts.length, media: cent(media(ts)) };
  }).filter((x) => x.dias >= 2 && x.media > 0);
  const ranking = [...porDiaSemana].sort((a, b) => b.media - a.media);
  const melhorDia = ranking[0] || null;
  const piorDia = ranking.length > 1 ? ranking[ranking.length - 1] : null;

  // Produto que subiu ou sumiu: últimos 30 dias contra os 30 anteriores.
  const qtdPorProduto = (ini, fim) => {
    const m2 = new Map();
    for (const v of vendas) {
      const d = v.data || '';
      ...
      for (const it of (v.itens || [])) {
        const q = num(it.qtd); if (q <= 0) continue;
        m2.set(it.nome || '—', (m2.get(it.nome || '—') || 0) + q);
      }
    }
    return m2;
  };
  // Mesma janela do "mais vendidos" logo acima, senão o mesmo produto aparece
  // com duas quantidades diferentes e o Darci se contradiz.
  const q30 = qtdPorProduto(desde, hoje);
  const q60 = qtdPorProduto(addDays(hoje, -61), addDays(hoje, -31));
  const mudouProduto = [];
  for (const [nome, qtd] of q30) {
    const antes = q60.get(nome) || 0;
    if (antes < 5) continue; // sem base de comparação não é notícia, é ruído
    const pct = pctDe(qtd, antes);
    if (Math.abs(pct) < 30) continue;
    mudouProduto.push({ nome, qtd, antes, pct });
  }
  for (const [nome, antes] of q60) {
    if (q30.has(nome) || antes < 5) continue; // vendia e parou de vender
    mudouProduto.push({ nome, qtd: 0, antes, pct: -100 });
  }
  mudouProduto.sort((a, b) => Math.abs(b.pct) - Math.abs(a.pct));
  const subiram = mudouProduto.filter((p) => p.pct > 0);
  const cairam = mudouProduto.filter((p) => p.pct < 0);

  // ===========================================================================
  // 2. AVISAR ANTES
  // "O gelo está abaixo do mínimo" avisa quando já está baixo. Aqui ele olha o
  // ritmo de consumo dos últimos dias e diz EM QUE DIA o item acaba — e cruza
  // isso com o fim de semana e com as mesas reservadas que já estão marcadas.
  // ===========================================================================
  const JANELA = 21;
  const iniRitmo = addDays(hoje, -JANELA);
  const diasEntre = (a, b) => Math.round((new Date(b + 'T12:00:00') - new Date(a + 'T12:00:00')) / 864e5);
  const ritmo = [];
  for (const it of estoque) {
    const movs = arr(it.movimentos);
    const saidas = movs.filter((m) => m && (m.tipo === 'saida' || m.tipo === 'venda') && (m.data || '') >= iniRitmo);
    const consumo = saidas.reduce((s, m) => s + num(m.qtd), 0);
    if (!(consumo > 0)) continue;
    // Item novo no PicoOS: divide pelos dias de vida dele, senão o ritmo sai
    // menor do que é de verdade e o aviso chega tarde.
    const maisAntigo = movs.map((m) => (m && m.data) || '').filter(Boolean).sort()[0] || iniRitmo;
    const desdeQuando = maisAntigo > iniRitmo ? maisAntigo : iniRitmo;
    const dias = Math.max(3, diasEntre(desdeQuando, hoje) || JANELA);
    const porDia = consumo / dias;
    if (!(porDia > 0)) continue;
    const saldo = num(it.saldo);
    const diasAte = Math.floor(saldo / porDia);
    ritmo.push({
      nome: it.nome || '—', saldo, unidade: it.unidade || 'un',
      porDia: Math.round(porDia * 100) / 100, diasAte, jaAcabou: saldo <= 0,
      acabaEm: addDays(hoje, Math.max(0, Math.min(365, diasAte))),
    });
  }
  ritmo.sort((a, b) => a.diasAte - b.diasAte);
  // O que já está zerado tem aviso próprio ("item zerado"), então aqui fica só
  // o que AINDA tem saldo — senão ele diz duas vezes a mesma coisa.
  const acabandoLogo = ritmo.filter((r) => r.diasAte <= 7 && r.saldo > 0);
  // Pra cruzar com as reservas, o que já zerou também conta: é o pior caso.
  const jaZerados = zerados.map((it) => ({ nome: it.nome || '—', saldo: 0, unidade: it.unidade || 'un', porDia: 0, diasAte: 0, jaAcabou: true, acabaEm: hoje }));
  const noFim = [...jaZerados, ...acabandoLogo.filter((r) => !zerados.some((z) => (z.nome || '') === r.nome))];

  // O que está pra vencer. Diferente do "acabando": aqui o produto existe, está
  // pago, e vira lixo se ninguém usar. É a perda mais boba que um bar tem.
  const vencendo = itensVencendo(estoque, hoje, 7);
  const jaVenceu = vencendo.filter((v) => v.dias < 0);

  // O próximo fim de semana (a sexta que vem), que é quando o bar enche.
  const proximaSexta = (() => { for (let i = 0; i <= 7; i++) { const d = addDays(hoje, i); if (diaSemanaNum(d) === 5) return d; } return addDays(hoje, 7); })();
  const faltaNoFds = acabandoLogo.filter((r) => r.acabaEm <= proximaSexta);

  // Mesas já reservadas, e o que pode faltar antes de cada uma delas.
  const reservasFuturas = reservas
    .filter((r) => r && r.data && r.data >= hoje)
    .sort((a, b) => `${a.data} ${a.hora || ''}`.localeCompare(`${b.data} ${b.hora || ''}`));
  const proximaReserva = reservasFuturas[0] || null;
  const pessoasReservadas = reservasFuturas
    .filter((r) => r.data <= addDays(hoje, 7))
    .reduce((s, r) => s + (num(r.pessoas) || 1), 0);
  const riscoReserva = [];
  for (const r of reservasFuturas.slice(0, 6)) {
    const itens = noFim.filter((x) => x.acabaEm <= r.data);
    if (itens.length) riscoReserva.push({ reserva: r, itens: itens.slice(0, 3) });
  }

  return {
    margens, noPrejuizo, altasCusto, publico,
    cmpOntem, cmpSemana, cmpMes, porDiaSemana, melhorDia, piorDia, subiram, cairam,
    recMesAte, despMesAte,
    ritmo, acabandoLogo, noFim, faltaNoFds, proximaSexta, vencendo, jaVenceu,
    reservasFuturas, proximaReserva, pessoasReservadas, riscoReserva,
    hoje, ontem, caixaOntem, fiadoOntem, totalOntem: Math.round((caixaOntem + fiadoOntem) * 100) / 100,
    recMes, despMes, resultado, maiorCat, aReceber, aReceberAgora, aReceberProximo, diaDoMes, devedores, noLimite,
    baixos, zerados, valorEstoque, vencidas, venceHoje, vence7, salarios, somaC,
...
// O que o Darci puxa a dona pelo braço pra ver assim que ela abre o PicoOS.
// Vem em ordem de urgência: o que dói hoje primeiro, o resto depois. Cada aviso
// carrega a pergunta que abre o assunto, pra ela tocar e ele explicar.
export function alertas(n) {
  const a = [];
  // `valor` vem separado do título de propósito: assim a tela consegue alinhar
  // os números numa coluna, em vez de enfiar tudo numa frase só.
  const põe = (id, nivel, titulo, valor, detalhe, pergunta) => a.push({ id, nivel, titulo, valor: valor || '', detalhe, pergunta });

  if (n.vencidas && n.vencidas.length) {
    põe('vencidas', 'urgente', plural(n.vencidas.length, 'conta vencida', 'contas vencidas'), brl(n.somaC(n.vencidas)),
      'Juro corre todo dia. Resolve essas primeiro.', 'Tenho conta pra pagar?');
  }
  if (n.salarios && n.salarios.length) {
    põe('salario', 'urgente', 'Pagamento de pessoal', brl(n.somaC(n.salarios)),
      `${plural(n.salarios.length, 'lançamento', 'lançamentos')} de salário/vale vencendo agora.`, 'Tenho conta pra pagar?');
  }
  if (n.venceHoje && n.venceHoje.length) {
    põe('hoje', 'urgente', 'Vence hoje', brl(n.somaC(n.venceHoje)),
      `${plural(n.venceHoje.length, 'conta', 'contas')} com vencimento hoje.`, 'Tenho conta pra pagar?');
  }
  if (n.zerados && n.zerados.length) {
    põe('zerados', 'urgente', `${plural(n.zerados.length, 'item zerado', 'itens zerados')} no estoque`, '',
      `${lista(n.zerados.map((i) => i.nome), 3)}. Venda perdida é a mais cara.`, 'O que está acabando?');
  }
  // Já venceu: é perda consumada, e cada dia parado é risco de alguém usar.
  if (n.jaVenceu && n.jaVenceu.length) {
    põe('venceu', 'urgente', `${plural(n.jaVenceu.length, 'item venceu', 'itens venceram')}`, '',
      `${lista(n.jaVenceu.map((v) => v.item.nome), 3)}. Tira do estoque e dá saída como vencido.`,
      'O que está vencendo?');
  }
  // Vence nos próximos dias: ainda dá pra salvar usando ou promovendo.
  const pertoDeVencer = (n.vencendo || []).filter((v) => v.dias >= 0);
  if (pertoDeVencer.length) {
    const v0 = pertoDeVencer[0];
    põe('vencendo', v0.dias <= 3 ? 'urgente' : 'atencao', `${v0.item.nome} ${textoValidade(v0.dias)}`, '',
      `${pertoDeVencer.length > 1 ? `E mais ${plural(pertoDeVencer.length - 1, 'item', 'itens')} essa semana. ` : ''}Usa, encaixa num prato ou faz promoção — é dinheiro que tu já pagou.`,
      'O que está vencendo?');
  }

  // Avisar ANTES: o que vai faltar num dia que já tem mesa reservada. Esse é o
  // pior de todos, porque a venda já está marcada na agenda.
  if (n.riscoReserva && n.riscoReserva.length) {
    const r0 = n.riscoReserva[0];
    const q = Math.round(Number(r0.reserva.pessoas) || 1);
    põe('reserva-estoque', 'urgente', `Mesa reservada ${quandoFalado(r0.reserva.data, n.hoje)} e item no fim`, '',
      `${lista(r0.itens.map((i) => i.nome), 3)} ${r0.itens.length === 1 ? 'acaba' : 'acabam'} antes — e são ${plural(q, 'pessoa', 'pessoas')} na mesa de ${r0.reserva.nome}. Compra antes.`,
      'O que vai acabar?');
  }
  // O que não chega no fim de semana, que é quando o bar enche.
  else if (n.faltaNoFds && n.faltaNoFds.length) {
    const f0 = n.faltaNoFds[0];
    põe('acaba-fds', 'atencao', `${f0.nome} acaba ${quandoFalado(f0.acabaEm, n.hoje)}`, '',
      `No ritmo dos últimos dias sai ${f0.porDia} ${f0.unidade} por dia e tu tem ${f0.saldo}. Isso é antes do fim de semana.${n.faltaNoFds.length > 1 ? ` Tem mais ${plural(n.faltaNoFds.length - 1, 'item assim', 'itens assim')}.` : ''}`,
      'O que vai acabar?');
  }
  else if (n.acabandoLogo && n.acabandoLogo.length) {
    const a0 = n.acabandoLogo[0];
    põe('acabando', 'atencao', `${a0.nome} acaba ${quandoFalado(a0.acabaEm, n.hoje)}`, '',
      `Sai ${a0.porDia} ${a0.unidade} por dia e tu tem ${a0.saldo}.${n.acabandoLogo.length > 1 ? ` Tem mais ${plural(n.acabandoLogo.length - 1, 'item acabando', 'itens acabando')} essa semana.` : ''}`,
      'O que vai acabar?');
  }
  const proximas = (n.vence7 || []).filter((c) => c.vencimento !== n.hoje);
  if (proximas.length) {
    põe('vence7', 'atencao', `${plural(proximas.length, 'conta', 'contas')} nos próximos dias`, brl(n.somaC(proximas)),
      'Deixa o caixa preparado.', 'Tenho conta pra pagar?');
  }
  if (n.baixos && n.baixos.length && !(n.zerados && n.zerados.length)) {
    põe('baixos', 'atencao', `${plural(n.baixos.length, 'item', 'itens')} abaixo do mínimo`, '',
      `${lista(n.baixos.map((i) => i.nome), 3)}. Repõe antes do fim de semana.`, 'O que está acabando?');
  }
  if (n.aReceberAgora > 0.005 && n.diaDoMes <= 10) {
    const faltam = 10 - n.diaDoMes;
    põe('fiado', 'atencao', 'Fiado pra receber', brl(n.aReceberAgora),
      faltam > 0 ? `O prazo é até o dia 10 — faltam ${plural(faltam, 'dia', 'dias')}.` : 'O prazo acaba hoje.', 'Quanto tenho a receber?');
  } else if (n.aReceberAgora > 0.005 && n.diaDoMes > 10) {
    põe('fiado', 'urgente', 'Fiado vencido', brl(n.aReceberAgora),
      'Passou do dia 10. Manda a cobrança pelo WhatsApp.', 'Quanto tenho a receber?');
  }
  if (n.noLimite && n.noLimite.length) {
    põe('limite', 'atencao', `${lista(n.noLimite.map((d) => d.nome), 2)} no limite do fiado`, '',
      'Não libera mais nada sem receber.', 'Quanto tenho a receber?');
  }
  if (n.noPrejuizo && n.noPrejuizo.length) {
    põe('prejuizo', 'urgente', `${lista(n.noPrejuizo.map((m) => m.nome), 2)} saindo no prejuízo`, '',
      'Vende por menos do que custa. Sobe o preço ou muda a ficha.', 'Alguma coisa subiu de preço?');
  }
  // Queda feia contra os dias iguais: isso não é notícia, é problema.
  const queda = !!(n.cmpOntem && n.cmpOntem.pct <= -25 && n.cmpOntem.dias >= 3);
  if (queda) {
    põe('queda', 'atencao', `Ontem foi ${Math.abs(n.cmpOntem.pct)}% abaixo do normal`, brl(n.cmpOntem.total),
      `${maiuscula(n.cmpOntem.dia)} costuma dar ${brl(n.cmpOntem.media)} aqui, na média de ${plural(n.cmpOntem.dias, 'dia igual', 'dias iguais')}. Vale entender o que foi.`,
      'Ontem foi bom ou ruim?');
  }
  // Notícia do dia: fecha com o que ela sempre quer saber — agora com referência,
  // porque número sozinho não diz se foi bom ou ruim. Se a queda já virou aviso
  // ali em cima, esse não repete.
  if ((n.caixaOntem > 0 || n.fiadoOntem > 0) && !queda) {
    const base = `${brl(n.caixaOntem)} no caixa${n.fiadoOntem > 0.005 ? ` e ${brl(n.fiadoOntem)} no fiado` : ''}.`;
    const c = n.cmpOntem;
    const ref = c && c.dias >= 2
      ? ` ${c.pct > 4 ? `${c.pct}% ACIMA` : c.pct < -4 ? `${Math.abs(c.pct)}% abaixo` : 'Na média'} do que ${c.dia} costuma dar (${brl(c.media)}).`
      : '';
    põe('ontem', 'info', 'Ontem', brl(n.totalOntem), base + ref, 'Ontem foi bom ou ruim?');
  }
  if (n.cmpSemana && Math.abs(n.cmpSemana.pct) >= 20 && n.cmpSemana.antes > 0) {
    const sobe = n.cmpSemana.pct > 0;
    põe('semana', 'info', `A semana veio ${sobe ? 'melhor' : 'mais fraca'}`, `${sobe ? '+' : ''}${n.cmpSemana.pct}%`,
      `Últimos sete dias: ${brl(n.cmpSemana.atual)}, contra ${brl(n.cmpSemana.antes)} nos sete anteriores.`,
      'Comparado com a semana passada?');
  }
  if (n.previsao && n.previsao.dias) {
    põe('previsao', 'info', 'Previsão de hoje', brl(n.previsao.total),
      `Faixa normal de ${brl(n.previsao.min)} a ${brl(n.previsao.max)}.`, 'Previsão de hoje');
  }
  return a;
}
...
export function recomendacoes(n) {
  ...
  // Antes do "está abaixo do mínimo" vem o que já dá pra prever que vai faltar.
  if (n.riscoReserva && n.riscoReserva.length) {
    const r0 = n.riscoReserva[0];
    const q = Math.round(Number(r0.reserva.pessoas) || 1);
    r.push(`${quandoFalado(r0.reserva.data, n.hoje)} tem mesa de ${plural(q, 'pessoa', 'pessoas')} (${r0.reserva.nome}) e ${lista(r0.itens.map((i) => i.nome), 3)} ${r0.itens.length === 1 ? 'acaba' : 'acabam'} antes disso. Compra hoje.`);
  } else if (n.faltaNoFds && n.faltaNoFds.length) {
    r.push(`${lista(n.faltaNoFds.map((x) => x.nome), 3)} não ${n.faltaNoFds.length === 1 ? 'chega' : 'chegam'} no fim de semana, no ritmo que está saindo. Repõe antes de sexta.`);
  } else if (!n.zerados.length && n.baixos.length) {
    r.push(`${plural(n.baixos.length, 'item', 'itens')} abaixo do mínimo. Repõe antes do fim de semana.`);
  }
  if (n.cmpOntem && n.cmpOntem.pct <= -25 && n.cmpOntem.dias >= 3) {
    r.push(`ontem veio ${Math.abs(n.cmpOntem.pct)}% abaixo do que ${n.cmpOntem.dia} costuma dar (${brl(n.cmpOntem.media)} contra ${brl(n.cmpOntem.total)}). Se repetir, é padrão, não azar.`);
  }
  const caiu0 = (n.cairam || [])[0];
  if (caiu0 && caiu0.pct <= -50) r.push(`${caiu0.nome} caiu ${Math.abs(caiu0.pct)}% de saída: era ${caiu0.antes} em 30 dias, agora é ${caiu0.qtd}. Ou o preço, ou o lugar no cardápio.`);
...
export function previsaoDoDia(n) {
  ...
  partes.push(`É a média ${p.dias === 1 ? 'do último dia igual a hoje' : `${p.dias === 8 ? 'das últimas oito' : `dos últimos ${p.dias}`} dias iguais a hoje`}: ${brl(p.caixa)} no caixa e ${brl(p.fiado)} no fiado${p.pedidos ? `, ${p.pedidos === 1 ? 'um pedido' : `uns ${p.pedidos} pedidos`}` : ''}${p.pessoas ? `, com ${p.pessoas === 1 ? 'uma pessoa' : `umas ${p.pessoas} pessoas`} no bar` : ''}.`);
...
// Como foi, comparado com o que costuma ser. É o que transforma um número
// numa notícia: R$ 1.400 não diz nada; "26% abaixo do teu sábado normal" diz.
export function comparativoTexto(n) {
  const p = [];
  if (n.cmpOntem) {
    const c = n.cmpOntem;
    const quanto = Math.abs(c.pct);
    const base = `${c.dia} aqui costuma dar ${brl(c.media)}, na média ${c.dias === 1 ? 'do último dia igual' : `dos últimos ${c.dias} dias iguais`}.`;
    p.push(
      quanto <= 4
        ? `Ontem tu fez ${brl(c.total)} — em cima da média. ${maiuscula(base)}`
        : c.pct > 0
          ? `Ontem tu fez ${brl(c.total)}, ${quanto}% ACIMA do normal: ${base} Dia bom.`
          : `Ontem tu fez ${brl(c.total)}, ${quanto}% abaixo do normal: ${base}`,
    );
  } else {
    p.push('Ainda não tenho dias iguais suficientes pra comparar ontem. Vai lançando o caixa que em duas ou três semanas eu te digo.');
  }
  if (n.cmpSemana && n.cmpSemana.antes > 0) {
    const s = n.cmpSemana;
    p.push(`Na semana: ${brl(s.atual)} nos últimos sete dias, contra ${brl(s.antes)} nos sete anteriores — ${s.pct > 0 ? `${s.pct}% a mais` : s.pct < 0 ? `${Math.abs(s.pct)}% a menos` : 'igualzinho'}.`);
  }
  if (n.cmpMes) {
    const m = n.cmpMes;
    p.push(`No mês, até o dia ${m.ateDia}: ${brl(m.rec)} de receita contra ${brl(m.recAnt)} no mesmo pedaço do mês passado (${m.pctRec > 0 ? `+${m.pctRec}%` : `${m.pctRec}%`}), e ${brl(m.desp)} de gasto contra ${brl(m.despAnt)} (${m.pctDesp > 0 ? `+${m.pctDesp}%` : `${m.pctDesp}%`}).`);
    if (m.pctRec < -10 && m.pctDesp > 10) p.push('Vendendo menos e gastando mais: essa é a combinação que mata bar. Olha as despesas essa semana.');
  }
  if (n.melhorDia && n.piorDia && n.melhorDia.dia !== n.piorDia.dia) {
    p.push(`Teu melhor dia é ${n.melhorDia.dia}, média de ${brl(n.melhorDia.media)}; o pior é ${n.piorDia.dia}, ${brl(n.piorDia.media)}.`);
  }
  const sub = (n.subiram || [])[0];
  const cai = (n.cairam || [])[0];
  if (sub) p.push(`Subindo: ${sub.nome}, ${sub.qtd} unidades nos últimos 30 dias contra ${sub.antes} nos 30 antes (+${sub.pct}%).`);
  if (cai) p.push(`Caindo: ${cai.nome}, de ${cai.antes} pra ${cai.qtd} (${cai.pct}%).${cai.pct <= -80 ? ' Esse praticamente parou de sair — vale ver se ainda faz sentido no cardápio.' : ''}`);
  return p.join(' ');
}

// O melhor e o pior dia da semana, sozinhos.
export function diasDaSemanaTexto(n) {
  const l = arr(n.porDiaSemana);
  if (!l.length) return 'Ainda não tenho semanas suficientes lançadas pra te dizer qual é teu melhor dia. Vai fechando as comandas e lançando o caixa.';
  const ord = [...l].sort((a, b) => b.media - a.media);
  const linha = ord.map((d) => `${d.dia} ${brl(d.media)}`).join(', ');
  const melhor = ord[0], pior = ord[ord.length - 1];
  const extra = ord.length > 1 && pior.media > 0 && melhor.media / pior.media >= 3
    ? ` ${maiuscula(melhor.dia)} rende ${Math.round(melhor.media / pior.media)} vezes o que ${pior.dia} rende. Vale pensar se ${pior.dia} compensa abrir, ou se pede uma promoção.`
    : '';
  return `Na média das últimas semanas: ${linha}. Teu melhor dia é ${melhor.dia}.${extra}`;
}

// Avisar ANTES: em que dia cada coisa acaba, no ritmo que está saindo.
export function antecipacaoTexto(n) {
  const l = arr(n.ritmo).filter((r) => r.diasAte <= 14);
  if (!arr(n.ritmo).length) {
    return 'Ainda não consigo calcular o ritmo: preciso de saídas de estoque registradas (venda por comanda ou baixa manual) pra saber quanto sai por dia.';
  }
  if (!l.length) {
    const prox = arr(n.ritmo)[0];
    return `Nada acabando nas próximas duas semanas. O primeiro a pedir reposição é ${prox.nome}, e ainda dura uns ${plural(prox.diasAte, 'dia', 'dias')}.`;
  }
  const p = [];
  const comoEsta = (r) => (r.jaAcabou
    ? `${r.nome} já está zerado (saía ${r.porDia} ${r.unidade} por dia)`
    : `${r.nome} acaba ${quandoFalado(r.acabaEm, n.hoje)} (sai ${r.porDia} ${r.unidade} por dia e tem ${r.saldo})`);
  p.push(`No ritmo dos últimos dias: ${l.slice(0, 5).map(comoEsta).join('; ')}.`);
  if (n.riscoReserva && n.riscoReserva.length) {
    const r0 = n.riscoReserva[0];
    const q = Math.round(Number(r0.reserva.pessoas) || 1);
    p.push(`E olha o cruzamento: ${quandoFalado(r0.reserva.data, n.hoje)} tem mesa reservada pra ${plural(q, 'pessoa', 'pessoas')} (${r0.reserva.nome}), e ${lista(r0.itens.map((i) => i.nome), 3)} ${r0.itens.length === 1 ? 'acaba' : 'acabam'} antes disso. Compra antes.`);
  } else if (n.faltaNoFds && n.faltaNoFds.length) {
    p.push(`${lista(n.faltaNoFds.map((r) => r.nome), 3)} não ${n.faltaNoFds.length === 1 ? 'chega' : 'chegam'} no fim de semana. Repõe antes de sexta.`);
  }
  return p.join(' ');
}

// O que está pra vencer, em ordem de urgência.
export function validadeTexto(n) {
  const l = arr(n.vencendo);
  if (!l.length) {
    return 'Nada vencendo nos próximos sete dias. Lembrando que só entra nessa conta o que tem data de validade preenchida no estoque — item sem data eu não tenho como vigiar.';
  }
  const venceu = l.filter((v) => v.dias < 0);
  const perto = l.filter((v) => v.dias >= 0);
  const p = [];
  if (venceu.length) p.push(`Já venceu: ${venceu.map((v) => `${v.item.nome} (${textoValidade(v.dias)})`).join(', ')}. Tira do estoque e dá saída como vencido, pra a perda entrar no teu controle.`);
  if (perto.length) p.push(`${venceu.length ? 'E vence' : 'Vence'} nos próximos dias: ${perto.map((v) => `${v.item.nome} ${textoValidade(v.dias)}`).join(', ')}.`);
  if (perto.length) p.push('Dá pra salvar: encaixa num prato do dia, numa promoção ou no consumo da casa. Vencido é dinheiro que tu já pagou e vai pro lixo.');
  return p.join(' ');
}
...
export function briefing(n) {
  ...
  if (n.caixaOntem > 0 || n.fiadoOntem > 0) {
    // Sempre com referência: o número sozinho não diz se foi bom ou ruim.
    const c = n.cmpOntem;
    const ref = c && c.dias >= 2
      ? ` Isso é ${Math.abs(c.pct) <= 4 ? 'em cima da' : c.pct > 0 ? `${c.pct}% acima da` : `${Math.abs(c.pct)}% abaixo da`} média de ${c.dia}, que é ${brl(c.media)}.`
      : '';
    p.push(`Ontem entrou ${brl(n.caixaOntem)} no caixa${n.fiadoOntem > 0.005 ? `, e mais ${brl(n.fiadoOntem)} ficaram no fiado — o dia fechou em ${brl(n.totalOntem)}` : ''}.${ref}`);
  } else {
    p.push('Ontem ainda não tem caixa lançado.');
  }
  if (n.recMes > 0 || n.despMes > 0) {
    const m = n.cmpMes;
    const ref = m ? ` Mês passado, até o dia ${m.ateDia}, tinha feito ${brl(m.recAnt)} — tu está ${m.pctRec > 0 ? `${m.pctRec}% acima` : m.pctRec < 0 ? `${Math.abs(m.pctRec)}% abaixo` : 'igual'}.` : '';
    p.push(`No mês tu fez ${brl(n.recMes)} de receita e gastou ${brl(n.despMes)}. ${n.resultado >= 0 ? `Está positivo em ${brl(n.resultado)}` : `Está negativo em ${brl(Math.abs(n.resultado))}`}.${ref}`);
  }
  if (n.previsao && n.previsao.dias) p.push(`Pra hoje eu espero uns ${brl(n.previsao.total)}.`);
  if (n.proximaReserva) {
    const q = Math.round(Number(n.proximaReserva.pessoas) || 1);
    p.push(`Tem mesa reservada ${quandoFalado(n.proximaReserva.data, n.hoje)}${n.proximaReserva.hora ? ` às ${n.proximaReserva.hora}` : ''}: ${n.proximaReserva.nome}, ${plural(q, 'pessoa', 'pessoas')}.`);
  }
...
function responderCru(texto, n) {
  ...
  // Comparar com o passado. Vem antes das outras porque "como foi ontem
  // comparado" tem que cair aqui, não na resposta seca de ontem.
  if (tem(t, 'melhor dia', 'pior dia', 'dia da semana', 'que dia rende', 'qual dia vende', 'dias da semana')) return diasDaSemanaTexto(n);
  if (tem(t, 'comparado', 'comparando', 'compara', 'comparacao', 'melhorou', 'piorou', 'foi bom ou ruim', 'bom ou ruim', 'foi bom', 'caiu', 'cresceu', 'aumentou o movimento', 'diminuiu', 'esta melhor', 'esta pior', 'em relacao', 'que o mes passado', 'que a semana passada', 'que o normal', 'dentro da media', 'acima da media', 'abaixo da media')) return comparativoTexto(n);
  // Avisar antes.
  // "vence" também é palavra de boleto ("tenho conta vencendo?"). Quando a
  // frase fala de conta, deixa passar pra resposta das contas lá embaixo.
  const ehDeConta = tem(t, 'conta', 'boleto', 'pagar', 'fornecedor', 'vencimento');
  if (!ehDeConta && tem(t, 'vencendo', 'vence', 'validade', 'vencido', 'estragar', 'estragando', 'perto de vencer', 'prazo')) return validadeTexto(n);
  if (tem(t, 'vai acabar', 'quando acaba', 'quanto dura', 'dura quanto', 'ate quando', 'preciso comprar', 'tenho que comprar', 'o que comprar', 'da pra quantos dias', 'quantos dias', 'ritmo', 'aguenta ate')) return antecipacaoTexto(n);
  ...
  if (tem(t, 'ontem', 'fechou ontem')) {
    ...
    const pesOntem = n.publico.de(n.ontem);
    const contou = pesOntem > 0 && !(pesOntem === n.pedidosOntem && n.pedidosOntem > 1);
    const gente = contou ? ` Passaram ${plural(pesOntem, 'pessoa', 'pessoas')} pelo bar, ${brl(n.publico.gastoDe(n.ontem) / pesOntem)} por pessoa.` : '';
    return `Ontem entrou ${brl(n.caixaOntem)} no caixa${n.fiadoOntem > 0.005 ? `, mais ${brl(n.fiadoOntem)} no fiado. O dia fechou em ${brl(n.totalOntem)}` : ''}.${n.pedidosOntem ? ` Foram ${plural(n.pedidosOntem, 'pedido', 'pedidos')}, ticket médio de ${brl(n.ticketOntem)}.` : ''}${gente}`;
  }
  ...
  if (tem(t, 'receber', 'fiado', 'devendo', 'deve', 'cobrar', 'devedor')) {
    ...
    // Separa o que já venceu (consumo dos meses passados) do que ainda corre.
    const ciclo = n.aReceberAgora > 0.005 && n.aReceberProximo > 0.005
      ? ` Desse total, ${brl(n.aReceberAgora)} é de quem tem conta do mês passado e ${n.diaDoMes <= 10 ? 'vence até o dia 10' : 'já passou do dia 10'}; ${brl(n.aReceberProximo)} é de quem só começou a consumir depois que o mês virou, e cai no ciclo que vem.`
      : n.aReceberAgora > 0.005
        ? ` Tudo isso entra na cobrança e ${n.diaDoMes <= 10 ? 'vence até o dia 10' : 'já passou do dia 10 — está atrasado'}.`
        : ' É tudo de gente que só começou a consumir depois que o mês virou — paga do dia 01 ao 10 do mês que vem.';
    return `Tu tem ${brl(n.aReceber)} pra receber, de ${plural(n.devedores.length, 'cliente', 'clientes')}. Os maiores: ${top}.${ciclo}${alerta} Cobrança se faz cedo, não na véspera.`;
  }
  ...
  return `${NAO_ENTENDI} Toca numa das sugestões aqui embaixo, ou me pergunta assim: a previsão de hoje, como foi ontem, quanto tenho a receber, o que está acabando, como está o mês, tenho conta pra pagar, o que mais vende, ou o que eu faço hoje.`;
}
