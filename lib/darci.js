...
export function interpretarComando(texto, { estoque = [], despesas = [] } = {}) {
  ...
  const falaDeTarefa = /^(anota|anotar|nota|lembra|lembre|lembrete|preciso|tenho que|tarefa|bota na lista|p[oõ]e na lista)\b/.test(t);
  ...
  // 2) Despesa: qualquer jeito de falar, desde que tenha um valor.
  ...
  const mTar = /^(?:anota|anotar|nota|lembra|lembre|lembrete|preciso|tenho que|tarefa|bota na lista|p[oõ]e na lista)\s+(?:que\s+|de\s+|do\s+|da\s+|pra\s+|para\s+)?(.+)$/i.exec(cru);
  if (falaDeTarefa && mTar && mTar[1].trim().length > 2) {
    return { tipo: 'tarefa', titulo: 'Anotar no TO DO', resumo: `Anotar no TO DO: "${maiuscula(limparNome(mTar[1]))}".`, dados: { texto: maiuscula(limparNome(mTar[1])) } };
  }
