...
export function analisarBar({ receitas = [], despesas = [], compras = [], vendas = [], estoque = [], tarefas = [], clientes = [], cardapio = [], fichas = [], desdeMs = 0 } = {}) {
  ...
  // O ciclo do fiado: cobra-se do dia 01 ao 10. Enquanto a pessoa não acerta, o
  // que ela deve é deste mês; depois que acerta, o que consumir fica pro próximo.
...
function responderCru(texto, n) {
  ...
  if (tem(t, 'receber', 'fiado', 'devendo', 'deve', 'cobrar', 'devedor')) {
    ...
    const ciclo = n.aReceberAgora > 0.005 && n.aReceberProximo > 0.005
      ? ` Desse total, ${brl(n.aReceberAgora)} é de quem ainda não acertou e ${n.diaDoMes <= 10 ? 'vence até o dia 10' : 'já passou do dia 10'}; ${brl(n.aReceberProximo)} é de quem já pagou e consumiu depois, então só cai mês que vem.`
      : n.aReceberAgora > 0.005
        ? ` Tudo isso ${n.diaDoMes <= 10 ? 'vence até o dia 10' : 'já passou do dia 10 — está atrasado'}.`
        : ' É tudo de gente que já acertou a conta e consumiu depois — só cai do dia 01 ao 10 do mês que vem.';
