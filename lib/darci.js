...
export function analisarBar({ receitas = [], despesas = [], compras = [], vendas = [], estoque = [], tarefas = [], clientes = [], cardapio = [], fichas = [], desdeMs = 0 } = {}) {
  ...
  // O fiado fecha como fatura de cartão: o consumo até o fim do mês passado é o
  // que se cobra agora (dia 01 ao 10); o deste mês já é da próxima fatura.
...
function responderCru(texto, n) {
  ...
  if (tem(t, 'receber', 'fiado', 'devendo', 'deve', 'cobrar', 'devedor')) {
    ...
    const ciclo = n.aReceberAgora > 0.005 && n.aReceberProximo > 0.005
      ? ` Desse total, ${brl(n.aReceberAgora)} é da fatura fechada e ${n.diaDoMes <= 10 ? 'vence até o dia 10' : 'já passou do dia 10'}; ${brl(n.aReceberProximo)} é consumo deste mês, que só cai no ciclo que vem.`
      : n.aReceberAgora > 0.005
        ? ` Tudo isso é da fatura fechada e ${n.diaDoMes <= 10 ? 'vence até o dia 10' : 'já passou do dia 10 — está atrasado'}.`
        : ' É tudo consumo deste mês — a fatura só fecha no fim do mês e se paga do dia 01 ao 10 do mês que vem.';
