...
export function analisarBar({ receitas = [], despesas = [], compras = [], vendas = [], estoque = [], tarefas = [], clientes = [], cardapio = [], fichas = [], desdeMs = 0 } = {}) {
  ...
  for (let i = 7; diasIguais.length < 8 && i <= 8 * 7; i += 7) {
    ...
    diasIguais.push({ data: d, caixa: cx, fiado: fi, total: cx + fi, pedidos: vs.length, pessoas: pes, itens: vs });
  }
...
export function alertas(n) {
  ...
  // Notícia do dia: fecha com o que ela sempre quer saber.
  if (n.caixaOntem > 0 || n.fiadoOntem > 0) {
    põe('ontem', 'info', 'Ontem', brl(n.totalOntem),
      `${brl(n.caixaOntem)} no caixa${n.fiadoOntem > 0.005 ? ` e ${brl(n.fiadoOntem)} no fiado` : ''}.`, 'Como foi ontem?');
  }
