...
// Os dias de escala do mês que já passaram (não conta hoje: o turno de hoje
// ainda pode acontecer, e cobrar por ele seria injusto).
export function diasDeEscala(jornada, hojeISO) {
  const dias = (jornada && Array.isArray(jornada.dias)) ? jornada.dias : [];
  if (!dias.length) return [];
  const [yy, mm] = String(hojeISO).split('-').map(Number);
  const diaHoje = Number(String(hojeISO).slice(8, 10));
  const out = [];
...
export function saldoDaPessoa(turnos, jornada, hojeISO) {
  ...
  const faltas = diasDeEscala(jornada, hojeISO).filter((d) => !comTurno.has(d));
