...
// TODOS os dias de escala do mês, tenham passado ou não. É a base do valor da
// hora: o salário paga o mês inteiro, não só o que já aconteceu.
export function diasDeEscalaDoMes(jornada, mes) {
  const dias = (jornada && Array.isArray(jornada.dias)) ? jornada.dias : [];
  if (!dias.length) return 0;
  let n = 0;
  for (let d = 1; d <= diasNoMes(mes); d++) {
    if (dias.includes(diaSemanaDe(`${mes}-${String(d).padStart(2, '0')}`))) n += 1;
  }
  return n;
}

// Quanto vale UMA hora naquele mês.
//
// Quem recebe por mês tem valor de hora VARIÁVEL, e isso não é detalhe: a
// cozinha (sex/sáb/dom, 8h) faz 96h em setembro e 112h em outubro pelo mesmo
// R$ 1.500. A hora vale R$ 15,63 num mês e R$ 13,39 no outro. Usar uma média
// fixa faria o desconto não fechar com o salário.
//
// Quem recebe por hora (valorHora) continua com o valor dela, fixo.
export function valorHoraDoMes(jornada, mes) {
  const sal = Number((jornada && jornada.salarioMes) || 0);
  if (sal > 0) {
    const horasMes = diasDeEscalaDoMes(jornada, mes) * horasJornada(jornada);
    return horasMes > 0 ? sal / horasMes : 0;
  }
  return Number((jornada && jornada.valorHora) || 0);
}
...
export function saldoAcumulado(turnos, jornada, hojeISO) {
  ...
  // O dinheiro é somado MÊS A MÊS, cada um pela hora daquele mês. Converter o
  // total de horas por uma taxa só daria errado quando os meses têm números
  // diferentes de dias de escala — que é o normal.
  const emDinheiro = comEscala.map((x) => emReais(x.s.saldo, jornada, x.mes)).filter((v) => v != null);
  return {
    meses: porMes,
    ...
    reais: emDinheiro.length === comEscala.length && comEscala.length
      ? Math.round(emDinheiro.reduce((s, v) => s + v, 0) * 100) / 100
      : null,
  };
}
...
// O banco de horas em dinheiro. Só responde quando o setor tem valor da hora
// configurado — sem ele, devolve null, e a tela mostra só as horas em vez de
// inventar um valor que ela usaria pra pagar alguém.
export function emReais(horas, jornada, mes) {
  const vh = valorHoraDoMes(jornada, mes);
  if (!(vh > 0) || horas == null || !Number.isFinite(horas)) return null;
  const v = horas * vh;
  // Arredonda pelo TAMANHO, não pelo sinal. Math.round() joga o meio centavo
  // sempre pra cima, então −85,245 virava −85,24 e +85,245 virava +85,25: a
  // mesma dívida valia diferente dependendo de quem devia a quem.
  const sinal = v < 0 ? -1 : 1;
  return sinal * (Math.round(Math.abs(v) * 100) / 100);
}
