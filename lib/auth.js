...
function tokenReservas() {
  const segredo = process.env.SESSION_SECRET || '';
  return crypto.createHash('sha256').update('pico-do-mane-reservas:' + segredo).digest('hex');
}

export function valorSessaoReservas() {
  return tokenReservas();
}
...
export function papelDaSessao(valorCookie) {
  ...
  if (igualConstante(valorCookie, tokenReservas())) return 'reservas';
  return null;
}
