...
export function engenhariaDoCardapio(linhas = []) {
  const itens = (linhas || []).filter((l) => l && l.lucro != null);
  if (itens.length < MIN_ITENS) {
    return { itens: [], grupos: [], poucosDados: true, motivo: `Precisa de pelo menos ${MIN_ITENS} produtos com ficha técnica pra comparar.`, cortes: null };
  }

  const totalQtd = itens.reduce((s, l) => s + num(l.qtd), 0);
  if (!(totalQtd > 0)) {
    return { itens: [], grupos: [], poucosDados: true, motivo: 'Nenhum desses produtos saiu no período escolhido.', cortes: null };
  }
...
export function engenhariaPorCategoria(linhas = []) {
  ...
  return { categorias: saida, forasteiras };
}
