...
export async function montarResumoDiario(sb) {
  ...
  const linhas = [];
  if (totalOntem > 0) linhas.push(`💰 Ontem: ${brl(totalOntem)} em ${vendasOntem.length} venda(s)`);
  if (agendaHoje.length) linhas.push(`📅 Hoje na agenda: ${agendaHoje.slice(0, 5).join(' · ')}${agendaHoje.length > 5 ? '…' : ''}`);
  if (vencemHoje.length) linhas.push(`🧾 ${vencemHoje.length} conta(s) vencem hoje (${brl(totalVence)})`);
  if (baixos.length) linhas.push(`📉 ${baixos.length} item(ns) no estoque mínimo: ${baixos.slice(0, 4).map((it) => it.nome).join(', ')}${baixos.length > 4 ? '…' : ''}`);
  if (noLimite.length) linhas.push(`💳 ${noLimite.length} fiado(s) no limite: ${noLimite.slice(0, 4).map((g) => g.nome).join(', ')}${noLimite.length > 4 ? '…' : ''}`);

  if (!linhas.length) return null;
  return { titulo: 'Bom dia! Resumo do PicoOS ☕', corpo: linhas.join('\n'), url: '/' };
}
