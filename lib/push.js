...
import { num, brl, todayISO, addDays, limparNome, fiadoDaVenda, abertoDaVenda } from './util';
...
export async function montarResumoDiario(sb) {
  ...
  const linhas = [];
  if (totalOntem > 0) {
    linhas.push(`Faturamento de ontem: ${brl(totalOntem)} (${plural(vendasOntem.length, 'venda', 'vendas')})`);
    // Quebra do que entrou no caixa vs. o que ficou no fiado (só se houver fiado).
    linhas.push(fiadoOntem > 0.005 ? `Recebido: ${brl(recebidoOntem)} · Fiado: ${brl(fiadoOntem)}` : `Recebido: ${brl(recebidoOntem)}`);
  }
