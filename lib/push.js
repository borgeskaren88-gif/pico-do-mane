...
import { num, brl, todayISO, addDays, limparNome, fiadoDaVenda, abertoDaVenda, diaOperacional, classificarFiado } from './util';
import { resumoDaReserva, pedePreparo, etiquetasDaReserva } from './reservas';
import { listarEventos } from './google';
import { itensVencendo, textoValidade } from './estoque';
...
export const AVISOS_PADRAO = {
  ...
  agenda: true,         // reserva/compromisso da agenda, na hora marcada
};
...
// Avisa o resultado da conferência do fechamento: saiu produto que ninguém
// cobrou? Só manda quando tem o que dizer — aviso de "está tudo certo" todo dia
// vira barulho e ela para de abrir.
//
// E só entra no aviso o que o sistema pode AFIRMAR (item contável, vendido
// direto). O duvidoso fica na tela, onde tem espaço pra explicar o porquê; num
// push de duas linhas ele viraria acusação sem defesa.
export async function notificarConferencia(sb, caixa) {
  try {
    const c = caixa && caixa.conferencia;
    if (!c || !c.totais) return { enviados: 0 };
    if (!(await lerAvisos(sb)).caixa) return { enviados: 0 };
    const certas = (c.faltas || []).filter((x) => x && x.nivel === 'certo' && num(x.falta) > 0);
    const faltaCaixa = num(c.totais.faltaNoCaixa);
    if (!certas.length && faltaCaixa <= 0.005) return { enviados: 0 };

    const partes = [];
    if (certas.length) {
      partes.push(certas.slice(0, 2).map((x) => `${x.falta} ${x.unidade} de ${x.nome}`).join(', ')
        + (certas.length > 2 ? ` e mais ${certas.length - 2}` : ''));
      const perdida = num(c.totais.receitaPerdidaCerta);
      if (perdida > 0.005) partes.push(`${brl(perdida)} que não entraram`);
    }
    if (faltaCaixa > 0.005) partes.push(`faltam ${brl(faltaCaixa)} na gaveta`);

    return await enviarPush(sb, {
      titulo: certas.length ? 'Saiu sem ser cobrado' : 'Diferença no caixa',
      corpo: partes.join(' · '),
      url: '/', tag: 'conferencia-' + caixa.id, audiencia: 'dona',
    });
  } catch { return { enviados: 0 }; }
}
...
// Avisa NA HORA da reserva/compromisso da agenda — a dona e o atendimento.
// Roda sempre que alguém com o app aberto pede (a cada poucos minutos) e no
// cron da manhã. Guarda o que já avisou, pra não repetir o mesmo evento.
// Mesa reservada: assim que a pessoa das reservas anota, TODO mundo sabe. A
// cozinha precisa se preparar, o atendimento precisa guardar a mesa e a dona
// precisa saber que o dia vai ter movimento.
export async function notificarReservaNova(sb, reserva) {
  if (!reserva || !reserva.data) return null;
  const [, mes, dia] = String(reserva.data).split('-');
  const quando = `${dia}/${mes}${reserva.hora ? ` às ${reserva.hora}` : ''}`;
  const quantos = Number(reserva.pessoas) || 1;
  const corpo = `${quando} · ${reserva.nome} · ${quantos} ${quantos === 1 ? 'pessoa' : 'pessoas'}`
    + (reserva.obs ? ` · ${String(reserva.obs).slice(0, 90)}` : '');
  try {
    return await enviarPush(sb, {
...
// Lembrete de confirmar a mesa: na VÉSPERA, a Mari recebe um aviso pra cada
// reserva que ainda não foi confirmada. Um aviso por reserva, uma vez só — a
// linha `avisosReservas` guarda o que já saiu.
//
// Sai a partir das 10h da manhã: reserva do dia seguinte avisada à meia-noite e
// cinco não serve pra nada, e ninguém confirma mesa de madrugada.
const CHAVE_RESERVAS = 'avisosReservas';
const HORA_MINIMA = 10;

const agoraBR = () => {
  try {
    const f = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Sao_Paulo', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', hour12: false });
...
export async function notificarReservasAmanha(sb) {
  const { dia, hora } = agoraBR();
  if (hora < HORA_MINIMA) return { ok: true, enviados: 0, motivo: 'cedo demais' };
  const amanha = addDays(dia, 1);

  const { data } = await sb.from('pdm_dados').select('valor').like('chave', 'reserva:%');
  const doDia = (data || []).map((r) => r.valor).filter((r) => r && r.data === amanha && !r.confirmada);
  if (!doDia.length) return { ok: true, enviados: 0, motivo: 'nada amanhã' };

  const { data: jaVisto } = await sb.from('pdm_dados').select('valor').eq('chave', CHAVE_RESERVAS).maybeSingle();
  const marcados = (jaVisto?.valor && typeof jaVisto.valor === 'object') ? { ...jaVisto.valor } : {};

  let enviados = 0;
  for (const r of doDia) {
    const chave = `${r.id}:${amanha}`;
    if (marcados[chave]) continue;
    // O corpo já traz as etiquetas (aniversário, bolo, alergia): a Mari escreve
    // a mensagem de confirmação a partir daqui, sem ter que abrir pra saber.
    const corpo = `${r.hora ? `${r.hora} · ` : ''}${resumoDaReserva(r)}`
      + (r.obs ? ` · ${String(r.obs).slice(0, 80)}` : '');
    try {
      await enviarPush(sb, {
        titulo: '📞 Confirmar a mesa de amanhã',
        corpo,
        // Leva direto na reserva, com o botão do WhatsApp à mão. Cair na tela
        // inicial e ter que procurar é o que faz o aviso virar barulho.
        url: '/?reserva=' + encodeURIComponent(r.id),
        tag: 'reserva-confirmar-' + r.id,
        audiencia: 'reservas',
      });
      marcados[chave] = new Date().toISOString();
      enviados += 1;
    } catch { /* um aviso que falhou não pode travar os outros */ }
  }
  if (enviados) {
    // Guarda só o que ainda interessa: o que é de ontem pra trás não volta.
    const limpos = {};
    for (const [k, v] of Object.entries(marcados)) { if (k.slice(-10) >= dia) limpos[k] = v; }
    await sb.from('pdm_dados').upsert({ chave: CHAVE_RESERVAS, valor: limpos, atualizado_em: new Date().toISOString() }, { onConflict: 'chave' });
  }
  return { ok: true, enviados };
}

// Avisa a DONA, na véspera, das mesas de amanhã que precisam de preparo:
// aniversário, bolo, alergia. Não é sobre confirmar a reserva (isso é da Mari)
// — é sobre dar tempo de comprar vela, separar espaço, avisar a cozinha.
//
// UM aviso por dia com todas juntas, não um por reserva: cinco pushes numa
// tarde fazem ela silenciar o app, e aí perde os que importam.
export async function notificarPreparoAmanha(sb) {
  const { dia, hora } = agoraBR();
  if (hora < HORA_MINIMA) return { ok: true, enviados: 0, motivo: 'cedo demais' };
  // 'agenda' é a chave que já liga/desliga reserva e compromisso na tela de
  // Notificações. Inventar uma nova aqui criaria um aviso que ela não consegue
  // desligar, porque nada na tela mexeria nele.
  if (!(await lerAvisos(sb)).agenda) return { ok: true, enviados: 0, motivo: 'desligado' };
  const amanha = addDays(dia, 1);

  const { data } = await sb.from('pdm_dados').select('valor').like('chave', 'reserva:%');
  const pedem = (data || []).map((r) => r.valor)
    .filter((r) => r && r.data === amanha && pedePreparo(r))
    .sort((a, b) => String(a.hora || '').localeCompare(String(b.hora || '')));
  if (!pedem.length) return { ok: true, enviados: 0, motivo: 'nada pra preparar' };

  const { data: jaVisto } = await sb.from('pdm_dados').select('valor').eq('chave', CHAVE_RESERVAS).maybeSingle();
  const marcados = (jaVisto?.valor && typeof jaVisto.valor === 'object') ? { ...jaVisto.valor } : {};
  const chave = `preparo:${amanha}`;
  if (marcados[chave]) return { ok: true, enviados: 0, motivo: 'já avisei hoje' };

  const linhas = pedem.map((r) => {
    const etq = etiquetasDaReserva(r).map((e) => e.texto).join(', ');
    return `${r.hora ? `${r.hora} ` : ''}${r.nome}${etq ? ` — ${etq}` : ''}`;
  });
  const r = await enviarPush(sb, {
    titulo: pedem.length === 1 ? 'Amanhã tem mesa pra preparar' : `Amanhã tem ${pedem.length} mesas pra preparar`,
    corpo: linhas.join(' · ').slice(0, 300),
    url: '/?reserva=' + encodeURIComponent(pedem[0].id),
    tag: 'preparo-' + amanha,
    audiencia: 'dona',
  });
  marcados[chave] = new Date().toISOString();
  ...
  for (const [k, v] of Object.entries(marcados)) { if (k.slice(-10) >= dia) limpos[k] = v; }
  await sb.from('pdm_dados').upsert({ chave: CHAVE_RESERVAS, valor: limpos, atualizado_em: new Date().toISOString() }, { onConflict: 'chave' });
  return { ok: true, enviados: r.enviados || 0, mesas: pedem.length };
}

// ---------------------------------------------------------------------------
// VALIDADE: um aviso por dia do que está vencendo. Vai pra dona e pra cozinha,
// que é quem mexe na geladeira.
//
// Um aviso por item por dia (a chave leva a data), pra não repetir o mesmo
// produto a cada vez que o app abre.
const CHAVE_VALIDADE = 'avisosValidade';

export async function notificarValidade(sb) {
  const { dia, hora } = agoraBR();
  if (hora < HORA_MINIMA) return { ok: true, enviados: 0, motivo: 'cedo demais' };

  const { data } = await sb.from('pdm_dados').select('valor').eq('chave', 'painel').maybeSingle();
  const estoque = Array.isArray(data?.valor?.estoque) ? data.valor.estoque : [];
  const lista = itensVencendo(estoque, dia, 3); // só o que aperta: 3 dias
  if (!lista.length) return { ok: true, enviados: 0, motivo: 'nada vencendo' };

  const { data: jaVisto } = await sb.from('pdm_dados').select('valor').eq('chave', CHAVE_VALIDADE).maybeSingle();
  const marcados = (jaVisto?.valor && typeof jaVisto.valor === 'object') ? { ...jaVisto.valor } : {};

  // Um aviso só, com tudo dentro: três avisos seguidos viram barulho e ela
  // aprende a ignorar.
  const novos = lista.filter((v) => !marcados[`${v.item.id}:${dia}`]);
  if (!novos.length) return { ok: true, enviados: 0, motivo: 'já avisei hoje' };

  const venceu = novos.filter((v) => v.dias < 0);
  const titulo = venceu.length ? '⚠️ Item vencido no estoque' : '⏳ Vencendo no estoque';
  // Diz QUANTO vence, não só o quê: "2 de Leite" é acionável, "Leite" faz ela
  // abrir o app pra descobrir se são 2 ou 20.
  const corpo = novos.slice(0, 4).map((v) => {
    const q = Number(v.qtd) > 0 ? `${Number(v.qtd)} ${v.item.unidade || 'un'} de ` : '';
    return `${q}${v.item.nome} ${textoValidade(v.dias)}`;
  }).join(' · ')
    + (novos.length > 4 ? ` · e mais ${novos.length - 4}` : '');

  let enviados = 0;
  try {
    await enviarPush(sb, { titulo, corpo, url: '/', tag: 'validade-' + dia, audiencia: 'dona' });
    await enviarPush(sb, { titulo, corpo, url: '/', tag: 'validade-' + dia, audiencia: 'cozinha' });
    enviados = 1;
    for (const v of novos) marcados[`${v.item.id}:${dia}`] = new Date().toISOString();
  } catch { /* aviso é bônus */ }

  if (enviados) {
    const limpos = {};
    for (const [k, val] of Object.entries(marcados)) { if (k.slice(-10) >= dia) limpos[k] = val; }
    await sb.from('pdm_dados').upsert({ chave: CHAVE_VALIDADE, valor: limpos, atualizado_em: new Date().toISOString() }, { onConflict: 'chave' });
  }
  return { ok: true, enviados };
}

const CHAVE_AGENDA = 'avisosAgenda';
const ANTES_MS = 15 * 60 * 1000; // avisa até 15 min antes de começar
const DEPOIS_MS = 20 * 60 * 1000; // e ainda avisa se ninguém abriu o app na hora

export async function notificarAgenda(sb) {
  try {
    if (!(await lerAvisos(sb)).agenda) return { enviados: 0 };
    const eventos = await listarEventos();
    if (!Array.isArray(eventos) || !eventos.length) return { enviados: 0 };
    const agora = Date.now();
    const naJanela = eventos.filter((e) => {
      if (!e || e.diaTodo || e.tarefa || !e.inicio) return false;
      const t = Date.parse(e.inicio);
      if (!t) return false;
      return (t - agora) <= ANTES_MS && (agora - t) <= DEPOIS_MS;
    });
    if (!naJanela.length) return { enviados: 0 };

    const { data } = await sb.from('pdm_dados').select('valor').eq('chave', CHAVE_AGENDA).maybeSingle();
    const jaAvisei = (data && data.valor && typeof data.valor === 'object') ? data.valor : {};
    const novos = naJanela.filter((e) => !jaAvisei[e.id]);
    if (!novos.length) return { enviados: 0 };

    let enviados = 0;
    for (const e of novos) {
      const hora = new Intl.DateTimeFormat('pt-BR', { timeZone: 'America/Sao_Paulo', hour: '2-digit', minute: '2-digit', hourCycle: 'h23' }).format(new Date(e.inicio));
      const falta = Math.round((Date.parse(e.inicio) - agora) / 60000);
      const corpo = falta > 1 ? `Começa às ${hora} — daqui a ${falta} min.` : `Começa agora, ${hora}.`;
      const aviso = { titulo: `Agenda: ${e.titulo}`, corpo, url: '/', tag: 'agenda-' + e.id };
      // Vai pros dois lados: quem atende no salão e a dona.
      const a = await enviarPush(sb, { ...aviso, audiencia: 'garcom' });
      const b = await enviarPush(sb, { ...aviso, audiencia: 'dona' });
      enviados += (a.enviados || 0) + (b.enviados || 0);
      jaAvisei[e.id] = agora;
    }
    // Guarda só os últimos 120 avisados, pra linha não crescer sem fim.
    const podado = Object.fromEntries(Object.entries(jaAvisei).sort((x, y) => y[1] - x[1]).slice(0, 120));
    await sb.from('pdm_dados').upsert({ chave: CHAVE_AGENDA, valor: podado, atualizado_em: new Date().toISOString() }, { onConflict: 'chave' });
    return { enviados, eventos: novos.length };
  } catch { return { enviados: 0 }; }
}
...
export async function montarResumoDiario(sb) {
  ...
  // Quanta gente esteve no bar ontem (vem do contador de pessoas da comanda).
  // Se deu exatamente uma por mesa, ninguém contou — melhor não dizer nada do
  // que dar número errado.
  const pessoasOntem = vendasOntem.reduce((s, v) => s + (num(v.pessoas) || 0), 0);
  if (pessoasOntem > 0 && !(pessoasOntem === vendasOntem.length && vendasOntem.length > 1)) {
    const porPessoa = totalOntem > 0 ? totalOntem / pessoasOntem : 0;
    linhas.push(`Ontem: ${pessoasOntem} ${pessoasOntem === 1 ? 'pessoa' : 'pessoas'}${porPessoa > 0 ? ` · ${brl(porPessoa)} por pessoa` : ''}`);
  }
  ...
  // Só entra na cobrança o consumo de MESES PASSADOS — o deste mês ainda nem
  // venceu (o ciclo é do dia 01 ao 10 do mês seguinte).
  const ymHoje = hoje.slice(0, 7);
  const idadeFiado = classificarFiado(vendas, ymHoje);
  const venceuAgora = abertos.filter((v) => idadeFiado.balde.get(v.id) !== 'proximo');
  const porClienteVencido = new Set(venceuAgora.map((v) => norm(v.nome)).filter(Boolean));
  const totalAberto = Math.round(venceuAgora.reduce((s, v) => s + abertoDaVenda(v), 0) * 100) / 100;
  if (diaDoMes >= 1 && diaDoMes <= 10 && totalAberto > 0.005) {
    const quantos = porClienteVencido.size;
