...
export async function montarResumoDiario(sb) {
  ...
  const linhas = [];
  // Dinheiro de ontem: prioriza o caixa fechado; se não houve, usa as comandas.
  const temCaixa = caixasOntem.length > 0;
  const dinheiroOntem = temCaixa ? caixaRecebidoOntem : recebidoOntem;
  if (dinheiroOntem > 0 || fiadoOntem > 0.005) {
    const base = `Caixa de ontem: ${brl(dinheiroOntem)}`;
    linhas.push(fiadoOntem > 0.005 ? `${base} · Fiado: ${brl(fiadoOntem)}` : base);
  }
