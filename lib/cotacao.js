...
// O preço da cotação NA UNIDADE DO ITEM de estoque.
//
// Existe pra comparar com o que ela está pagando na nota, e o que se compara
// tem que estar na mesma régua. A cotação do chopp é "R$ 345 o barril, 30 L
// dentro" = R$ 11,50 o litro; a nota está em litro. Comparar 11,50 com 345
// dizia "abaixo da menor cotação" quando ela estava pagando EXATAMENTE o
// cotado — e diria a mesma coisa se ela pagasse R$ 20 o litro, que é quase o
// dobro. O erro apontava sempre pro lado de "tá barato", que é o pior lado.
//
// Devolve null quando não dá pra passar de uma unidade pra outra: aí a tela
// não compara, em vez de comparar errado.
export function precoNaUnidadeDoItem(cotacao, item, fatorEntre) {
  const unit = precoUnitario(cotacao);
  if (!(unit > 0) || !item) return null;
  const de = (cotacao && cotacao.embalagem) || 'un';
  const para = item.unidade || 'un';
  const f = fatorEntre(de, para);
  // fatorEntre(de, para) diz quantos "para" cabem em 1 "de". O preço anda ao
  // contrário: 1 L custa 1/1000 do que custam 1000 ml.
  if (f == null || !(f > 0)) return null;
  return Math.round((unit / f) * 10000) / 10000;
}
