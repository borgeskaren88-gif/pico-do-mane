...
import { qtdNaUnidadeDoItem, custoPelasCompras, indexarCompras, chaveNome } from './estoque';
...
export function cmvDoMes({ vendas = [], fichas = [], estoque = [], cardapio = [], mes } = {}) {
  ...
  for (const v of vendas || []) {
    ...
    for (const item of (v.itens || [])) {
      ...
      const g = porCategoria.get(cat) || { categoria: cat, qtd: 0, custo: 0, receita: 0 };
      g.qtd += r.qtd; g.custo = cent(g.custo + r.custo); g.receita = cent(g.receita + receita);
...
export function estoqueNoMes(estoque = [], compras = [], mes) {
  ...
  return { estoque: itens, semBase, comBase: itens.length - semBase };
}
