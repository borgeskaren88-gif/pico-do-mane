...
// A idade do fiado, pela regra do Pico — que funciona como fatura de cartão: a
// conta FECHA no fim do mês. O que a pessoa consumiu até o último dia do mês
// passado é o que se cobra agora (do dia 01 ao 10). O que ela consome a partir
// do dia 1º entra na próxima fatura, mesmo que ela ainda deva a anterior.
//
// Ex.: a Jessica deve de agosto e pegou mais no dia 05 — só o de agosto pode ser
// cobrado agora. O Henry pagou hoje e pediu mais duas: essas são da próxima.
export function classificarFiado(vendas, ymAtual) {
  ...
  // Última vez que cada cliente pôs dinheiro na conta — não muda a fatura, mas
  // é bom mostrar na tela ("já acertou dia 09").
  ...
  const balde = new Map(); // id da venda -> 'agora' (fatura fechada) | 'proximo'
  for (const v of lista) {
    const ymV = String((v && (v.data || v.fechadaEm)) || '').slice(0, 7);
    balde.set(v.id, (ymV && ymV < ymAtual) ? 'agora' : 'proximo');
  }
