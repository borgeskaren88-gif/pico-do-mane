...
// Reparte o dinheiro de uma compra entre as parcelas do boleto.
//
// A ideia que faz isso funcionar sem mexer em nenhuma outra conta do sistema:
// a compra parcelada vira uma linha POR ITEM E POR PARCELA, e cada linha leva
// só a fatia de dinheiro daquela parcela. Assim toda soma que já existe
// (fornecedor, DRE, previsão, contas a pagar) continua batendo sozinha — o
// total das linhas é o total da nota, nem um centavo a mais.
//
// O que tem que fechar EXATO é a coluna: a soma dos itens dentro de uma parcela
// tem que dar o valor do boleto daquela parcela, porque é isso que ela paga.
// Centavo que sobra do arredondamento vai pro item de maior valor, que é onde
// menos se nota — é a regra do "maior resto".
//
// `totais`: quanto cada item custa na compra inteira (em reais).
// `valores`: quanto vale cada parcela (em reais).
// Devolve uma matriz [parcela][item] com o dinheiro de cada linha.
export function ratearParcelas(totais, valores) {
  const t = (totais || []).map((x) => Math.round(num(x) * 100));
  const somaT = t.reduce((s, x) => s + x, 0);
  return (valores || []).map((v) => {
    const alvo = Math.round(num(v) * 100);
    if (somaT <= 0) {
      // Compra sem valor: reparte igual, só pra não dividir por zero.
      const base = Math.floor(alvo / Math.max(1, t.length));
      const linha = t.map(() => base);
      if (linha.length) linha[linha.length - 1] += alvo - base * linha.length;
      return linha.map((c) => c / 100);
    }
    const bruto = t.map((x) => (x * alvo) / somaT);
    const linha = bruto.map((x) => Math.floor(x));
    let falta = alvo - linha.reduce((s, x) => s + x, 0);
    // Os centavos que sobraram vão pros maiores restos, um a um.
    const ordem = bruto
      .map((x, i) => ({ i, resto: x - Math.floor(x), peso: t[i] }))
      .sort((a, b) => b.resto - a.resto || b.peso - a.peso);
    for (let k = 0; falta > 0 && ordem.length; k++, falta--) linha[ordem[k % ordem.length].i] += 1;
    return linha.map((c) => c / 100);
  });
}
...
export function agruparContasAbertas(abertas) {
  ...
  for (const d of abertas) {
    ...
    // O VENCIMENTO entra na chave: uma nota parcelada é a mesma nota com três
    // boletos, e ela paga um de cada vez. Agrupando só por nota, as três
    // parcelas viravam um pagamento só e "marcar pago" quitava as três.
    const chave = notaTrim
      ? `n:${notaTrim}|${(d.fornecedor || '').trim().toLowerCase()}|${d.vencimento || ''}`
      : `i:${d.id}`;
    let g = map.get(chave);
