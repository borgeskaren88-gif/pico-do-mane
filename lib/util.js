...
// A idade do fiado, pela regra do Pico: enquanto a pessoa NÃO acerta a conta,
// tudo que ela deve é deste mês (é o que se cobra do dia 01 ao 10). Depois que
// ela acerta, o que consumir dali em diante já fica pro mês que vem.
//
// Ex.: o Henry pagou hoje e pediu mais duas cervejas depois — essas duas vão
// pro mês que vem, e ele não aparece mais como "devendo agora".
export function classificarFiado(vendas, ymAtual) {
  ...
  // Última vez que cada cliente pôs dinheiro na conta (quitando ou abatendo).
  ...
  for (const v of lista) {
    const k = chaveDe(v);
    const pago = ultimoPag.get(k) || '';
    const acertouNoCiclo = !!pago && pago.slice(0, 7) === ymAtual;
    const dataV = String((v && (v.data || v.fechadaEm)) || '').slice(0, 10);
    // Mesmo dia do acerto conta como depois: se ainda está em aberto, é porque
    // veio DEPOIS de ela pagar (o que era de antes foi quitado).
    balde.set(v.id, (acertouNoCiclo && dataV >= pago) ? 'proximo' : 'agora');
  }
  return { balde, ultimoPag, chaveDe };
}
