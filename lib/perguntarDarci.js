...
export async function testarIA() {
  try {
    ...
    const j = await r.json();
    if (j && j.semChave) return { estado: 'sem-chave' };
    if (j && j.ok) return { estado: 'ok', motor: j.motor, modelo: j.modelo, ms: j.ms };
    return { estado: 'erro', motor: j?.motor, modelo: j?.modelo, erro: j?.erro || 'Não consegui testar.', cru: j?.cru || '', status: j?.status || 0 };
  } catch {
    return { estado: 'erro', erro: 'Não consegui nem chegar no servidor. Vê se tem internet.' };
  }
}
