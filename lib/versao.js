...
// JOGA FORA A CÓPIA DO APP GUARDADA NESTE APARELHO.
//
// O app não vem num arquivo só: vem em pedaços, e a página diz o nome de cada
// um. Quando sai versão nova os pedaços trocam de nome — e se o aparelho estiver
// com a PÁGINA de antes e os PEDAÇOS de agora, falta metade do programa e o app
// inteiro cai com "Application error: a client-side exception has occurred".
//
// Recarregar sem limpar podia servir a mesma página velha de novo, então este
// passo vem ANTES de qualquer recarga por troca de versão.
//
// Não apaga dado nenhum: venda, compra e estoque moram no servidor. E NÃO
// desinstala o service worker, só manda ele buscar a versão nova de si mesmo —
// desinstalar desligaria os avisos que chegam no celular dela.
export async function limparCopiaGuardada() {
  try {
    if (typeof caches !== 'undefined') {
      const nomes = await caches.keys();
      await Promise.all(nomes.map((n) => caches.delete(n)));
    }
  } catch { /* se nem isso der, ainda vale recarregar */ }
  try {
