...
import { C, Card, Btn, Field, Label, inputStyle, Empty, Icone } from './ui';
...
export default function Habitos({ usuario }) {
  ...
  const adicionar = async (e) => {
    ...
    setSalvando(true);
    await acao({ acao: 'habitoAdd', nome: nome.trim() });
            ...
            return (
              <Card key={h.id} style={{ marginBottom: 8, padding: 14 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 30, display: 'flex', justifyContent: 'center', flexShrink: 0, color: s > 0 ? C.amber : C.faint }}><Icone name="flame" size={22} /></div>
                  ...
                  {g.eu && (
                    <button onClick={() => marcar(h.id, hoje, !feitoHoje)} title={feitoHoje ? 'Feito hoje' : 'Marcar hoje'}
                      style={{ width: 46, height: 46, borderRadius: 12, flexShrink: 0, cursor: 'pointer', border: `2px solid ${feitoHoje ? C.green : C.line}`, background: feitoHoje ? C.green : 'transparent', color: feitoHoje ? '#052014' : C.faint, fontSize: 24, fontWeight: 400, lineHeight: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {feitoHoje ? <Icone name="check" size={24} /> : '+'}
                    </button>
                  )}
                </div>
                  ...
                  {ultimos7.map((dia) => {
                    ...
                    return (
                      <button key={dia} disabled={!g.eu} onClick={() => g.eu && marcar(h.id, dia, !feito)}
                        ...
                        <span style={{ width: '100%', maxWidth: 34, aspectRatio: '1', borderRadius: 8, background: feito ? C.green : C.panel2, border: `1px solid ${ehHoje ? C.accent : C.line}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#052014' }}>
                          {feito ? <Icone name="check" size={15} /> : ''}
                        </span>
                      </button>
                    );
                  })}
