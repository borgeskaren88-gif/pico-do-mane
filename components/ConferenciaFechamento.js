...
export default function ConferenciaFechamento({
  ...
  dinheiroFinal = null, contado = '', onResultado,
}) {
  ...
  }, [estoque, hoje, verTodos]);

  const vendasHoje = useMemo(() => vendas.filter((v) => v && v.data === hoje), [vendas, hoje]);

  const resultado = useMemo(() => conferirFechamento({
    estoque, fichas, cardapio, vendas: vendasHoje, contagens,
    caixa: dinheiroFinal == null ? null : { dinheiroFinal, contado },
    hoje,
  }), [estoque, fichas, cardapio, vendasHoje, contagens, dinheiroFinal, contado, hoje]);

  // Entrega o resultado pro Caixa mandar junto no fechamento.
  React.useEffect(() => { if (onResultado) onResultado(resultado); }, [resultado, onResultado]);
  ...
  return (
    <div style={{ marginTop: 12 }}>
      ...
      <div style={{ fontSize: 12, color: C.muted, marginBottom: 10, lineHeight: 1.5 }}>
        Conta na prateleira o que tu quiser conferir e digita aqui. Eu comparo com o que as vendas de hoje explicam.
        O que tu não contar, eu não palpito.
      </div>
              ...
              <div key={it.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', borderTop: `1px solid ${C.hair}` }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: C.faint }}>
                    sistema: {numQtd(it.saldo)} {it.unidade || 'un'}
                    {girou > 0 ? ` · girou ${Math.round(girou * 1000) / 1000} hoje` : ''}
                  </div>
                </div>
                <div style={{ width: 96, flexShrink: 0 }}>
                  <QtdInput value={contagens[it.id] ?? ''} onChange={setC(it.id)} placeholder="contei" />
                </div>
                <div style={{ width: 96, flexShrink: 0, textAlign: 'right', fontSize: 11, fontWeight: 700, color: cor }}>
                  {linha ? (
                    <>
